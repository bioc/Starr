...
Only uniquely matching probes are included in the new annotation file. The genomic sequences must be provided by a \Rpackage{BSgenome} annotation package 
(see the \Rpackage{BSgenome} vignette on how to build such a package with your own set of sequences). \\
The example below, shows how to use the function for the given bpmap file from above. 
First we load the \Rpackage{BSgenome.Scerevisiae.UCSC.sacCer2} and then call the \Rfunction{remap} function. 
...
newbpmap <- remap(bpmapChr1, nseq=500000, bsg=Scerevisiae, reverse_complementary=TRUE)
@

In this case, 86.99 \% of the probe sequences could be mapped to a unique position in the genome. The method returns a list in the output format of the \Rmethod{affy} function \Rmethod{readBpmap}. 
...
<<Writing binary bpmap file, echo=TRUE, results=hide, eval=FALSE>>=
...
Please note, that this bpmap file differs from the original file. Consequently, one has to read in the data using this file, otherwise the \Rclass{probeAnno} object will not be compatible with the \Rclass{ExpressionSet}.
The procedure efficiently works for smaller genomes like \textit{S. cerevisiae} or \textit{Drosophila melangoster}. Searching larger genomes, like \textit{H. sapiens} can be very memory-consuming, so make sure to provide enough. 
If the memory on your computer is not sufficient, it could be better to use other programs like Mummer. The \Rpackage{Ringo} package contains R scripts to parse its output.
...
rpb3_ratio_smooth <- computeRunningMedians(rpb3_loess_ratio, probeAnno=probeAnnoChr1, allChr = "chr1", winHalfSize = 80)
