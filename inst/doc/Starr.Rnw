...
plotScatter(rpb3Chr1, density=T)
@ 
...
After quality assessment, we perform normalization of the raw data. Here we use the rank percentile normalization, which was proposed by Buck and Lieb \cite{rankpercentile}.
<<echo=TRUE, results=hide>>=
rpb3_rankpercentile <- normalize.Probes(rpb3Chr1, method="rankpercentile")
@

Besides this normalization method, there are e.g. loess, scale, quantile, vsn, MAT and some others available.
...
plotMA(rpb3_rankpercentile, ip=ips, control=controls)
@ 
...
rpb3_rankpercentile_ratio <- getRatio(rpb3_rankpercentile, ips, controls, description, fkt=median, featureData=F)
@
...
par(mfcol=c(1,2))
plotGCbias(exprs(rpb3_rankpercentile_ratio)[,1], featureData(rpb3_rankpercentile)$seq, main="")
plotPosBias(exprs(rpb3_rankpercentile_ratio)[,1], featureData(rpb3_rankpercentile)$seq, ylim=c(-0.5,0.5))
@ 
...
Besides the typical ChIP-chip analysis of the data, like visualization or peak finding - which are made available by \Rpackage{Ringo} - \Rpackage{Starr} provides
...
\Rpackage{Starr} provides an easy-to-use method for building new bpmap annotation files with remapped probe sequences. 
It implements the Aho-Corasick algorithm, which is designed for searching a given set of sequences in a text. 
Only uniquely matching probes are included in the new annotation file. The genomic sequences must be provided as fasta files. Each file is supposed to contain one chromosome. 
...
The example below, shows how to match the sequences of the given bpmap file from above to chromosome 1 of \textit{S. cerevisiae}. 
The sequences in this bpmap file are taken from both strands in $5' \rightarrow 3'$ direction, that means we have to search the +1 and -1 strand. 
...
In this case, 89.5 \% of the probe sequences could be mapped to a unique position on chromosome 1. The method returns a list in the output format of the \Rmethod{affy} function \Rmethod{readBpmap}. 

<<Summary of bpmap, echo=TRUE>>=
str(newbpmap)
@

One can use this list either to write a new binary bpmap file, or to create a new \Rclass{probeAnno} object. 


\subsection{Analyze the correlation of ChIP signals to other data}
...
<<means,echo=TRUE, print=FALSE, results=hide>>=
means_rpb3 <- getMeans(rpb3_rankpercentile_ratio, probeAnnoChr1, transcriptAnno[which(transcriptAnno$name %in% filteredIDs),], info)
@
...
<<echo=TRUE, print=FALSE, results=hide>>=
profile <- getProfiles(rpb3_rankpercentile_ratio, probeAnnoChr1, tssAnno, 500, 500, feature="TSS", borderNames="TSS", method="basewise")
@
...
In this section we shortly present how functions from the package \Rmethod{Ringo} can be used for further data analysis.
Note that the rank percentile normalization should not be used, if peak-finding is applied. This is only done to illustrate the functions of the package.
...
rpb3_ratio_smooth <- computeRunningMedians(rpb3_rankpercentile_ratio, probeAnno=probeAnnoChr1, allChr = "chr1", winHalfSize = 80)
sampleNames(rpb3_ratio_smooth) <- paste(sampleNames(rpb3_rankpercentile_ratio),"smoothed")
